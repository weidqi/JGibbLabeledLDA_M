...
public class Model {
    ...
    public double alpha;       // LDAEstimator hyperparameters
    public double beta = 0.01; // LDAEstimator hyperparameters
    ...
    public Model(LDAOption option) throws Exception {
        this(option, null);
    }

    public void initInfLDAOption(LDAOption option){
    ...
    public Model(LDAOption option, Model trnModel) throws Exception {
        modelName = option.modelName;
        K = option.K;
     ...
     * @throws Exception
     */
    public Model(LDAOption option, String[] docs, Model trnModel) throws Exception {
        ...
        // read in data
        data.readDataSet(docs);
    }

    /**
     * 预测单篇文章的最大主题
     */
    public int[] predictTopic() {
        int[] topic = new int[M];
        for (int i = 0; i < M; i++) {
            int MaxTopicId = -1;
            double MaxtopicProb = 0.0d;
            for (int j = 0; j < K; j++) {
                if (theta[i][j] > MaxtopicProb) {
                    MaxtopicProb = theta[i][j];
                    MaxTopicId = j;
                }
            }
            topic[i] = MaxTopicId;
     ...
     * Init parameters for estimation or inference
     */
    public boolean initDefaultModel() {
        if (!loadModel()) {
            System.out.println("Fail to load word-topic assignment file of the model!");
            return false;
        }
